...
class MissionOutcomeRepositoryTest extends IntegrationTestSupport {
	...
	@DisplayName("미션 ID와 미션 날짜로 미션결과를 조회한다.")
	@Test
	void findByMissionIdAndMissionDate() {
		// given
		User user = createUser("user@daum.net", "user1234!", SnsType.KAKAO);
		Mission mission = createMission(user, "운동하기", UNCHECKED, LocalTime.of(19, 0));
		MissionOutcome missionOutcome = createMissionOutcome(mission, LocalDate.of(2023, 10, 25), SUCCEED);
		...
		// when
		MissionOutcome foundMissionOutcome = missionOutcomeRepository.findByMissionIdAndMissionDate(
			mission.getId(),
			LocalDate.of(2023, 10, 25)
		);

		// then
		assertThat(foundMissionOutcome).isNotNull()
			.extracting("missionStatus", "missionDate")
			.containsExactly(SUCCEED, LocalDate.of(2023, 10, 25));
		assertThat(foundMissionOutcome.getMission().getId()).isEqualTo(mission.getId());
	}
